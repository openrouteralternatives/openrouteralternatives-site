...
| Helicone | Funding | Funding databases disagree on the round count (StartupIntros records two rounds totalling about $2.2M; Crunchbase exposes one). Two rounds are shown with an unresolved status; the acquisition by Mintlify is not counted as a round. |
...
level, licence/repository on products documented as closed source, and funding on the
three hyperscaler products (Amazon Bedrock, Azure AI Foundry, Google Vertex AI) and on
Envoy AI Gateway, because a corporation's fundraising history is not the funding of one
product and a community project has no funding profile.
